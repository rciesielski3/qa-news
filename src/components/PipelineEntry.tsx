...
import { getCategoryLabel, categoryTextColorClass, categoryBgColorClass, categoryBgColorClassDark, getCategoryColor } from '@/lib/styles';
  ...
  return (
    <li className="flex gap-3 py-3 border-b border-ink-200 dark:border-ink-600 group hover:bg-ink-50 dark:hover:bg-ink-800 transition-colors">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className={`flex-shrink-0 flex items-center justify-center text-xs font-mono uppercase tracking-widest no-underline px-3 py-1 rounded transition-opacity ${categoryBgColorClass(article.category)} ${categoryBgColorClassDark(article.category)} ${categoryTextColorClass(article.category)} hover:opacity-90`}
      >
        Read
      </a>
    </li>

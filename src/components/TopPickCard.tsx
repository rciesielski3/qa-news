...
export default function TopPickCard({ article, rank }: TopPickCardProps) {
  return (
    <div className="flex flex-col justify-between min-h-[280px] rounded-lg border border-ink-200 bg-paper p-6 dark:bg-ink-800 dark:border-ink-600">
      {/* Header: rank, category, top pick badge */}
      <div className="flex-shrink-0 flex items-center justify-start gap-3 mb-4 w-full">
      ...
      <div className="flex-grow flex flex-col w-full">
        ...
        {/* Subtitle or Summary (conditional) */}
        {article.subtitle ? (
          <p className="text-sm text-ink-600 mb-3 dark:text-paper-muted w-full line-clamp-2">
            {article.subtitle}
          </p>
        ) : article.summary ? (
          <p className="text-sm leading-relaxed text-ink-600 mb-4 dark:text-paper-muted w-full line-clamp-3">
            {article.summary}
          </p>
        ) : null}
      </div>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className={`flex-shrink-0 inline-flex items-center justify-center rounded h-8 px-4 font-mono text-xs uppercase tracking-widest no-underline transition-opacity ${categoryBgColorClass(article.category)} ${categoryBgColorDarkInverted(article.category)} ${categoryTextColorClass(article.category)} ${categoryTextColorDarkInverted(article.category)} hover:opacity-90`}
      >
        Read Article →
      </a>
    </div>
